...
def test(coverage):
    args = []
    if env.lower() != 'test':
        print("Not running in TEST env, try setting the environment to test: MYTEMPLATE_ENV=test")

    if coverage:
        args.extend(["--cov-report=term-missing", "--cov=mytemplate"])
